            ...
            <div class="card">
                <div class="card-header d-flex justify-content-between">
                    <div class="header-title">
                        <h4 class="card-title">Rol en el Permiso</h4>
                    </div>
                </div>
                ...
                <div class="card-body">
                    ...
                    @csrf
                                ...
                                <select class="form-control @error('role') is-invalid @enderror" id="role_id" name="role_id" required>
                                    ...
                                    @foreach ($roles as $role)
                                        <option value="{{ $role->id }}" 
                                            @if(in_array($role->id, $rolesWithPermissions ?? [])) disabled @endif>
                                            {{ $role->name }}
                                            @if(in_array($role->id, $rolesWithPermissions ?? [])) (ya asignado) @endif
                                        </option>
                                    @endforeach
                                </select>
                                @error('role_id')
                        ...
                        <div class="mt-2">
                            ...
                            <a class="btn" style="background-color: #ff6f6f; color: white;" href="{{ route('permission.index') }}">Cancelar</a>
                        </div>
                    </form>
                </div>
            </div>
