...
Route::middleware(['permission:attendence.menu'])->group(function () {
    ...
    Route::delete('/attendence/{date}', [AttendenceController::class, 'destroy'])->name('attendence.destroy');
});
...
// Routes for database backup operations
Route::prefix('dashboard/backup')->name('backup.')->group(function () {
    Route::get('/', [DatabaseBackupController::class, 'index'])->name('index'); // Muestra la lista de backups
    Route::post('/create', [DatabaseBackupController::class, 'create'])->name('create'); // Crea un nuevo backup
    Route::get('/download/{fileName}', [DatabaseBackupController::class, 'download'])->name('download'); // Descarga un backup
    Route::get('/delete/{fileName}', [DatabaseBackupController::class, 'delete'])->name('delete'); // Elimina un backup
});
...
Route::middleware(['permission:roles.menu'])->group(function () {
    ...
    Route::get('/roles', [RoleController::class, 'roleIndex'])->name('roles.index');
